...
class EmotionallyIntelligentPromptFramework {
  constructor() {
    ...
    this.inputSchema = Joi.object({
      ...
      primaryGoal: Joi.string().regex(/^[a-zA-Z0-9\s,.$%&()-]{5,100}$/).required(),
      brandVoice: Joi.string().valid('warm', 'bold', 'optimistic', 'professional', 'playful', 'inspirational', 'custom').required(),
      customTone: Joi.string().regex(/^[a-zA-Z0-9\s]{1,50}$/).when('brandVoice', { is: 'custom', then: Joi.required() }),
      businessDescription: Joi.string().min(10).max(500).required(),
      socialPlatforms: Joi.string().when('templateType', { is: 'socialMedia', then: Joi.required() }),
      contentStrategy: Joi.string().when('templateType', { is: 'socialMedia', then: Joi.required() }),
      contentSource: Joi.string().uri().when('templateType', { is: 'websiteAudit', then: Joi.required() }),
      auditScope: Joi.string().when('templateType', { is: 'websiteAudit', then: Joi.required() })
    }).unknown(true);
    ...
    this.emotionalDrivers = {
      ...
      businessContext: {
        family_business: ["generational", "legacy-building", "family values", "tradition", "heritage"],
        tech_startup: ["scalable", "data-driven", "user-centric", "agile", "innovative"],
        local_business: ["community-rooted", "neighborhood-focused", "locally-sourced", "personal", "intimate"],
        b2b_service: ["partnership-focused", "results-driven", "strategic", "collaborative", "growth-oriented"],
        creative_agency: ["artistic", "expressive", "original", "inspiring", "boundary-pushing"]
      },
      audienceContext: {
        families: ["safety-focused", "value-driven", "community-minded", "future-planning", "nurturing"],
        professionals: ["efficiency-focused", "growth-oriented", "network-building", "career-advancing", "time-conscious"],
        entrepreneurs: ["risk-taking", "opportunity-seeking", "independence-valued", "innovation-driven", "resilience-focused"],
        creatives: ["self-expression", "authenticity-seeking", "inspiration-driven", "uniqueness-valued", "artistic"]
      }
    };
    ...
    this.culturalContext = {
      "Denver, CO": {
        values: ["outdoor lifestyle", "craft culture", "community-minded", "environmentally conscious", "work-life balance"],
        businessContext: ["local sourcing", "sustainability focus", "craft brewing culture", "outdoor recreation", "tech-friendly"],
        demographicInsights: ["health-conscious", "environmentally aware", "tech-savvy", "active lifestyle", "community-engaged"],
        localReferences: ["Mile High City", "Rocky Mountains", "craft beer scene", "outdoor recreation", "tech hub"]
      },
      "Austin, TX": {
        values: ["keep it weird", "music culture", "entrepreneurial spirit", "food scene", "tech innovation"],
        businessContext: ["creative industries", "tech startups", "music venues", "food trucks", "SXSW culture"],
        demographicInsights: ["creative professionals", "young professionals", "music lovers", "foodie culture", "tech workers"],
        localReferences: ["Keep Austin Weird", "SXSW", "food truck culture", "live music", "tech corridor"]
      },
      default: {
        values: ["quality", "community", "innovation", "growth", "authenticity"],
        businessContext: ["customer-focused", "growth-oriented", "quality-driven", "service-excellence"],
        demographicInsights: ["value-conscious", "quality-seeking", "relationship-focused", "results-oriented"],
        localReferences: []
      }
    };
  }
  ...
  validateInputs(inputData, templateType) {
    const { error } = this.inputSchema.validate({ ...inputData, templateType }, { abortEarly: false });
    if (error) {
  ...
  async storeTemplate(templateType, version, content) {
    if (typeof process === 'undefined' || !process.env || !process.env.SUPABASE_URL || !process.env.SUPABASE_KEY) return;
  ...
  async getLatestTemplateVersion(templateType) {
    if (typeof process === 'undefined' || !process.env || !process.env.SUPABASE_URL || !process.env.SUPABASE_KEY) return null;
    const { createClient } = await import('@supabase/supabase-js');
    const supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_KEY);
...
4. **PostPurchase Object**: Confirmation email, feedback prompt, follow-up email, share option
  ...
  buildEmotionalContext(inputData) {
    const brandDrivers = this.emotionalDrivers.brandVoice[inputData.brandVoice?.toLowerCase()] || [];
    const businessType = this.inferBusinessType(inputData.businessDescription);
    const businessDrivers = this.emotionalDrivers.businessContext[businessType] || [];
    const audienceType = this.inferAudienceType(inputData.targetAudience);
    const audienceDrivers = this.emotionalDrivers.audienceContext[audienceType] || [];
...
- Inferred Emotional Themes: ${this.inferEmotionalThemes(inputData)}
  ...
  buildCulturalContext(inputData) {
    ...
    const context = this.culturalContext[location] || this.culturalContext.default;
...
- Local References: ${context.localReferences.join(', ')}
  ...
  inferBusinessType(description) {
    const keywords = {
      ...
      local_business: ['local', 'community', 'neighborhood', 'bakery', 'restaurant'],
      b2b_service: ['consulting', 'services', 'business', 'enterprise', 'corporate'],
      creative_agency: ['design', 'creative', 'agency', 'marketing', 'advertising']
    };
  ...
  inferAudienceType(audience) {
    const keywords = {
      ...
      entrepreneurs: ['entrepreneurs', 'business owners', 'startups', 'founders'],
  ...
  inferEmotionalThemes(inputData) {
    const themes = [];
    if (inputData.businessDescription?.includes('community')) themes.push('community connection');
    if (inputData.primaryGoal?.includes('funding')) themes.push('growth ambition');
    if (inputData.brandVoice === 'warm') themes.push('authentic relationships');
    if (inputData.targetAudience?.includes('families')) themes.push('family values');
  ...
  validateOutput(output) {
    ...
    if (!output.CoreContent && !output.Plan && !output.Campaign && !output.Audit) {
  ...
  async generateCompletePrompt(inputData, templateType, customization = {}) {
    ...
    await this.storeTemplate(templateType, customization.version || '1.0.0', systemPrompt.join('\n'));
