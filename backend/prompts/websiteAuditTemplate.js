...
class WebsiteAuditTemplate extends EmotionallyIntelligentPromptFramework {
  ...
  async generateWebsiteAudit(inputData) {
    ...
    const requiredFields = [
      ...
      'brandVoice', 'businessDescription', 'contentSource', 'auditScope'
    ];
    ...
    const enhancedUserPrompt = this.enhanceWebsiteAuditPrompt(promptData.user, inputData);
...
- Response time: <1.5s (PRD Section 6.3).
  ...
  validateWebsiteAuditOutput(output) {
    ...
    if (!output.Audit) {
      ...
      if (!output.Audit.CanAI_Output) websiteAuditErrors.push('Missing Audit.CanAI_Output');
      if (!output.Audit.Generic_Output) websiteAuditErrors.push('Missing Audit.Generic_Output');
      if (typeof output.Audit.TrustDelta !== 'number' || output.Audit.TrustDelta < 4.2) {
        websiteAuditErrors.push('Invalid or low Audit.TrustDelta (must be ≥4.2)');
      }

      // Validate word count
      const canAIWordCount = output.Audit.CanAI_Output ? output.Audit.CanAI_Output.split(/\s+/).length : 0;
      const genericWordCount = output.Audit.Generic_Output ? output.Audit.Generic_Output.split(/\s+/).length : 0;
      if (canAIWordCount < 300 || canAIWordCount > 400) {
        websiteAuditErrors.push(`CanAI_Output: Invalid word count (${canAIWordCount}, must be 300–400)`);
      }
    ...
    if (output.PostPurchase) {
      ...
      if (missingPostPurchase.length > 0) {
        websiteAuditErrors.push(`Missing PostPurchase fields: ${missingPostPurchase.join(', ')}`);
      }
    }
  ...
  getTechTrendExample() {
    return {
      inputData: {
        ...
        businessDescription: 'A tech startup offering AI-driven analytics tools in Denver',
        contentSource: 'https://techtrend.com',
        auditScope: 'UX, accessibility, conversions'
      },
      expectedOutput: {
        Summary: {
          Summary: 'Audit for TechTrend\'s website to boost conversions with innovative, Denver-focused UX.',
          ConfidenceScore: 0.96,
          ClarifyingQuestions: []
        },
        Audit: {
          CanAI_Output: '[300–400-word audit with emotional, cultural UX recommendations]',
          Generic_Output: '[300–400-word technical audit]',
          TrustDelta: 4.6
        },
        PostPurchase: {
          ConfirmationEmail: 'Thank you, [userName]! Your TechTrend audit is ready. Access now.',
          PDFDownload: 'https://supabase.com/files/audit-12345.pdf',
          FeedbackPrompt: 'Does this audit align with your vision? Rate 1–5. [Open-ended]',
          FollowUpEmail: 'How\'s your TechTrend website update? Refine with CanAI.',
